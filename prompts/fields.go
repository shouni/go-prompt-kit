...
// 対になる関数です。プロンプトのカタログ表示や、入力フォームの組み立てにも使えます。
//
...
// フィールド名を data からの位置として報告できないからです（本体の外にある
// {{range .Items}} の .Items 自体や、else 節、$ を起点にした {{$.Language}} は
// 位置が確定するので含まれます）。列挙されたものは確かに要求されていますが、
// 列挙されなかったものが不要とは限りません。
//
